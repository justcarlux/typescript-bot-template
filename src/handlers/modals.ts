...
export default async (client: Bot, interaction: ModalSubmitInteraction) => {
    ...
    if (interaction.channel && !interaction.channel.isDMBased()) {
        logger.run(
            `${interaction.user.tag} (ID: ${interaction.user.id}) sent a modal dialog with the name of "${modal.name}" in #${interaction.channel?.name} (ID: ${interaction.channel?.id}) from the server "${interaction.guild?.name}" (ID: ${interaction.guild?.id})`,
            {
                color: "cyan",
                ignore: !getConfig().enable.modalInteractionLogs,
