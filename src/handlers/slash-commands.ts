...
export default async (client: Bot, interaction: ChatInputCommandInteraction) => {
    ...
    if (!command) return;
    if (!getConfig().owners.includes(interaction.user.id) && command.authorization) {
        const authorization = command.authorization(interaction);
    ...
    if (interaction.options.getSubcommand(false)) {
        ...
        if (!getConfig().owners.includes(interaction.user.id) && subCommand.authorization) {
            const authorization = subCommand.authorization(interaction);
            if (!authorization.authorized && authorization.payload) {
                return await interaction.reply(authorization.payload);
            }
        }
    ...
    if (interaction.channel && !interaction.channel.isDMBased()) {
        logger.run(`Execution of "/${command.data.name}" by ${interaction.user.tag} (ID: ${interaction.user.id}) in #${interaction.channel?.name} (ID: ${interaction.channel?.id}) from the guild "${interaction.guild?.name}" (ID: ${interaction.guild?.id}) took ${((Date.now() - timestamp) / 1000).toFixed(1)}s (${(Date.now() - timestamp)} ms)`, {
            color: "cyan", category: "Slash Commands", ignore: !getConfig().enable.slashCommandExecutionTimeLogs
        });
    }
